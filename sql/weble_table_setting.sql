...
create table if not exists host (
    host_id bigint AUTO_INCREMENT not null PRIMARY KEY,
    customer_id bigint not null,
...
create table if not exists cancel_reservation (
    cancel_id BIGINT AUTO_INCREMENT not null PRIMARY KEY,
...
create table if not exists feedback_customer (
	feedback_customer_id BIGINT AUTO_INCREMENT NOT NULL PRIMARY KEY,
    customer_id BIGINT NOT NULL,
    host_id BIGINT NOT NULL,
    title VARCHAR(200),
    content VARCHAR(1000),
...
create table if not exists feedback_host (
	feedback_host_id BIGINT AUTO_INCREMENT NOT NULL PRIMARY KEY,
    customer_id BIGINT NOT NULL,
    host_id BIGINT NOT NULL,
    title VARCHAR(200),
    content VARCHAR(1000),
...
BEGIN
    SET @sql = CONCAT('CREATE TABLE ', customer_id, '_delete_customer (', customer_id, ' BIGINT PRIMARY KEY, request_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL)');
    PREPARE s1 FROM @sql;
    EXECUTE s1;
...
BEGIN
    SET @sql = CONCAT('CREATE TABLE ', customer_id, '_bookmark (', customer_id, ' BIGINT PRIMARY KEY, rental_id BIGINT NOT NULL)');
    PREPARE s1 FROM @sql;
    EXECUTE s1;
    DEALLOCATE PREPARE s1;
END$$
...
BEGIN
    SET @sql = CONCAT('CREATE TABLE ', customer_id, '_banned_customer (', customer_id, ' BIGINT PRIMARY KEY, blocked_date TIMESTAMP NOT NULL, unlocked_date TIMESTAMP NOT NULL, locked_at VARCHAR(30) NOT NULL)');
    PREPARE s1 FROM @sql;
    EXECUTE s1;
    DEALLOCATE PREPARE s1;
END$$
...
create table if not exists notification (
	notification_id BIGINT AUTO_INCREMENT not null PRIMARY KEY,
    customer_id BIGINT not null,
